...
{
	return (state <= SCE_PAWN_COMMENTDOC) ||
		// including SCE_PAWN_DEFAULT, SCE_PAWN_COMMENT, SCE_PAWN_COMMENTLINE
		(state == SCE_PAWN_COMMENTLINEDOC) || (state == SCE_PAWN_COMMENTDOCKEYWORD) ||
		(state == SCE_PAWN_COMMENTDOCKEYWORDERROR);
}
...
static bool IsStreamCommentStyle(int style) {
	return style == SCE_PAWN_COMMENT ||
		style == SCE_PAWN_COMMENTDOC ||
		style == SCE_PAWN_COMMENTDOCKEYWORD ||
		style == SCE_PAWN_COMMENTDOCKEYWORDERROR;
}
...
static const char *const pawnWordLists[] = {
            "Native keywords",
            "Forward keywords",
			"Statements keywords",
			"Constants keywords",
...
struct OptionSetPawn : public OptionSet<OptionsPawn> {
	OptionSetPawn() {
		DefineProperty("styling.within.preprocessor", &OptionsPawn::stylingWithinPreprocessor,
			"For C++ code, determines whether all preprocessor code is styled in the "
			"preprocessor style (0, the default) or only from the initial # to the end "
			"of the command word(1).");

		DefineProperty("lexer.cpp.allow.dollars", &OptionsPawn::identifiersAllowDollars,
			"Set to 0 to disallow the '$' character in identifiers with the cpp lexer.");

		DefineProperty("lexer.cpp.track.preprocessor", &OptionsPawn::trackPreprocessor,
			"Set to 1 to interpret #if/#else/#endif to grey out code that is not active.");

		DefineProperty("lexer.cpp.update.preprocessor", &OptionsPawn::updatePreprocessor,
			"Set to 1 to update preprocessor definitions when #define found.");

		DefineProperty("fold", &OptionsPawn::fold);

		DefineProperty("fold.comment", &OptionsPawn::foldComment,
			"This option enables folding multi-line comments and explicit fold points when using the C++ lexer. "
			"Explicit fold points allows adding extra folding by placing a //{ comment at the start and a //} "
			"at the end of a section that should fold.");

		DefineProperty("fold.cpp.comment.explicit", &OptionsPawn::foldCommentExplicit,
			"Set this property to 0 to disable folding explicit fold points when fold.comment=1.");

		DefineProperty("fold.preprocessor", &OptionsPawn::foldPreprocessor,
			"This option enables folding preprocessor directives when using the C++ lexer. "
			"Includes C#'s explicit #region and #endregion folding directives.");

		DefineProperty("fold.compact", &OptionsPawn::foldCompact);

		DefineProperty("fold.at.else", &OptionsPawn::foldAtElse,
			"This option enables C++ folding on a \"} else {\" line of an if statement.");

		DefineWordListSets(pawnWordLists);
	}
};

class LexerPawn : public ILexer {
	bool caseSensitive;
	...
	OptionsPawn options;
	OptionSetPawn osPawn;
public:
	LexerPawn(bool caseSensitive_) :
	...
	const char * SCI_METHOD PropertyNames() {
		return osPawn.PropertyNames();
	}
	int SCI_METHOD PropertyType(const char *name) {
		return osPawn.PropertyType(name);
	}
	const char * SCI_METHOD DescribeProperty(const char *name) {
		return osPawn.DescribeProperty(name);
	}
	int SCI_METHOD PropertySet(const char *key, const char *val);
	const char * SCI_METHOD DescribeWordListSets() {
		return osPawn.DescribeWordListSets();
	}
	...
	static ILexer *LexerFactoryPawn() {
		return new LexerPawn(true);
	}
...
int SCI_METHOD LexerPawn::PropertySet(const char *key, const char *val) {
	if (osPawn.PropertySet(&options, key, val)) {
		return 0;
	}
	return -1;
}

int SCI_METHOD LexerPawn::WordListSet(int n, const char *wl) {
...
void SCI_METHOD LexerPawn::Lex(unsigned int startPos, int length, int initStyle, IDocument *pAccess) {
	...
	int chPrevNonWhite = ' ';
	int visibleChars = 0;
	int styleBeforeDCKeyword = SCE_PAWN_DEFAULT;
	bool continuationLine = false;
	bool isIncludePreprocessor = false;

	int lineCurrent = styler.GetLine(startPos);
	if (initStyle == SCE_PAWN_PREPROCESSOR) {
	...
	if (startPos > 0) {
		...
		while (--back && IsSpaceEquiv(styler.StyleAt(back)))
			;
		if (styler.StyleAt(back) == SCE_PAWN_OPERATOR) {
	...
	for (; sc.More(); sc.Forward()) {
		...
		if (sc.atLineStart) {
			if (sc.state == SCE_PAWN_STRING) {
				// Prevent SCE_PAWN_STRINGEOL from leaking back to previous line which
				// ends with a line continuation by locking in the state upto this position.
				sc.SetState(SCE_PAWN_STRING);
			}
		...
		switch (sc.state & maskActivity) {
			case SCE_PAWN_OPERATOR:
				sc.SetState(SCE_PAWN_DEFAULT|activitySet);
				break;
			case SCE_PAWN_NUMBER:
				// We accept almost anything because of hex. and number suffixes
				if (!setWord.Contains(sc.ch)) {
					sc.SetState(SCE_PAWN_DEFAULT|activitySet);
				}
				break;
			case SCE_PAWN_IDENTIFIER:
				if (!setWord.Contains(sc.ch) || (sc.ch == '.')) {
					...
					if (keywordsNative.InList(s)) {
						sc.ChangeState(SCE_PAWN_NATIVE|activitySet);
					} else if (keywordsForward.InList(s)) {
						sc.ChangeState(SCE_PAWN_FORWARD|activitySet);
					} else if (keywordsStatement.InList(s)) {
						sc.ChangeState(SCE_PAWN_STATEMENT|activitySet);
					} else if (keywordsConstant.InList(s)) {
						sc.ChangeState(SCE_PAWN_CONSTANT|activitySet);
					} else if (keywords.InList(s)) {
						sc.ChangeState(SCE_PAWN_WORD|activitySet);
					} else if (keywords2.InList(s)) {
						sc.ChangeState(SCE_PAWN_WORD2|activitySet);
					}

					sc.SetState(SCE_PAWN_DEFAULT|activitySet);
				}
				break;
			case SCE_PAWN_PREPROCESSOR:
				...
				break;
			case SCE_PAWN_COMMENT:
				if (sc.Match('*', '/')) {
					sc.Forward();
					sc.ForwardSetState(SCE_PAWN_DEFAULT|activitySet);
				}
				break;
			case SCE_PAWN_COMMENTDOC:
				if (sc.Match('*', '/')) {
					sc.Forward();
					sc.ForwardSetState(SCE_PAWN_DEFAULT|activitySet);
					...
					if ((IsASpace(sc.chPrev) || sc.chPrev == '*') && (!IsASpace(sc.chNext))) {
						styleBeforeDCKeyword = SCE_PAWN_COMMENTDOC;
						sc.SetState(SCE_PAWN_COMMENTDOCKEYWORD|activitySet);
					}
				}
				break;
			case SCE_PAWN_COMMENTLINE:
				if (sc.atLineStart) {
					sc.SetState(SCE_PAWN_DEFAULT|activitySet);
				}
				break;
			case SCE_PAWN_COMMENTLINEDOC:
				if (sc.atLineStart) {
					sc.SetState(SCE_PAWN_DEFAULT|activitySet);
				} else if (sc.ch == '@' || sc.ch == '\\') { // JavaDoc and Doxygen support
					// Verify that we have the conditions to mark a comment-doc-keyword
					if ((IsASpace(sc.chPrev) || sc.chPrev == '/' || sc.chPrev == '!') && (!IsASpace(sc.chNext))) {
						styleBeforeDCKeyword = SCE_PAWN_COMMENTLINEDOC;
						sc.SetState(SCE_PAWN_COMMENTDOCKEYWORD|activitySet);
					}
				}
				break;
			case SCE_PAWN_COMMENTDOCKEYWORD:
				if ((styleBeforeDCKeyword == SCE_PAWN_COMMENTDOC) && sc.Match('*', '/')) {
					sc.ChangeState(SCE_PAWN_COMMENTDOCKEYWORDERROR);
					sc.Forward();
					sc.ForwardSetState(SCE_PAWN_DEFAULT|activitySet);
					...
					if (!IsASpace(sc.ch) || !keywords2.InList(s + 1)) {
						sc.ChangeState(SCE_PAWN_COMMENTDOCKEYWORDERROR|activitySet);
					}
					sc.SetState(styleBeforeDCKeyword);
				}
				break;
			case SCE_PAWN_STRING:
				if (sc.atLineEnd) {
					sc.ChangeState(SCE_PAWN_STRINGEOL|activitySet);
			...
			case SCE_PAWN_CHARACTER:
				...
				break;
			case SCE_PAWN_STRINGEOL:
				if (sc.atLineStart) {
					sc.SetState(SCE_PAWN_DEFAULT|activitySet);
				}
				break;
			case SCE_PAWN_VERBATIM:
				if (sc.ch == '\"') {
					if (sc.chNext == '\"') {
						sc.Forward();
					} else {
		...
		if ((sc.state & maskActivity) == SCE_PAWN_DEFAULT) {
			if (sc.Match('@', '\"')) {
				sc.SetState(SCE_PAWN_VERBATIM|activitySet);
				sc.Forward();
			} else if (IsADigit(sc.ch) || (sc.ch == '.' && IsADigit(sc.chNext))) {
				sc.SetState(SCE_PAWN_NUMBER|activitySet);
			} else if (setWordStart.Contains(sc.ch) || (sc.ch == '@')) {
				sc.SetState(SCE_PAWN_IDENTIFIER|activitySet);
			} else if (sc.Match('/', '*')) {
				if (sc.Match("/**") || sc.Match("/*!")) {	// Support of Qt/Doxygen doc. style
					sc.SetState(SCE_PAWN_COMMENTDOC|activitySet);
				} else {
					sc.SetState(SCE_PAWN_COMMENT|activitySet);
				}
				sc.Forward();	// Eat the * so it isn't used for the end of the comment
			} else if (sc.Match('/', '/')) {
				if ((sc.Match("///") && !sc.Match("////")) || sc.Match("//!"))
					// Support of Qt/Doxygen doc. style
					sc.SetState(SCE_PAWN_COMMENTLINEDOC|activitySet);
				else
					sc.SetState(SCE_PAWN_COMMENTLINE|activitySet);
			} else if (sc.ch == '\"') {
				sc.SetState(SCE_PAWN_STRING|activitySet);
				isIncludePreprocessor = false;	// ensure that '>' won't end the string
			} else if (isIncludePreprocessor && sc.ch == '<') {
				sc.SetState(SCE_PAWN_STRING|activitySet);
			} else if (sc.ch == '\'') {
				sc.SetState(SCE_PAWN_CHARACTER|activitySet);
			} else if (sc.ch == '#' && visibleChars == 0) {
				// Preprocessor commands are alone on their line
				sc.SetState(SCE_PAWN_PREPROCESSOR|activitySet);
					...
					if (options.trackPreprocessor) {
						if (sc.Match("ifdef") || sc.Match("ifndef")) {
							...
							if (!preproc.CurrentIfTaken()) {
								...
								if (!activitySet)
									sc.ChangeState(SCE_PAWN_PREPROCESSOR|activitySet);
				...
				sc.SetState(SCE_PAWN_OPERATOR|activitySet);
			}
		}
...
void SCI_METHOD LexerPawn::Fold(unsigned int startPos, int length, int initStyle, IDocument *pAccess) {
	...
	for (unsigned int i = startPos; i < endPos; i++) {
		...
		if (options.foldComment && IsStreamCommentStyle(style)) {
			if (!IsStreamCommentStyle(stylePrev) && (stylePrev != SCE_PAWN_COMMENTLINEDOC)) {
				levelNext++;
			} else if (!IsStreamCommentStyle(styleNext) && (styleNext != SCE_PAWN_COMMENTLINEDOC) && !atEOL) {
				// Comments don't end at end of line and the next character may be unstyled.
				levelNext--;
			}
		}
		if (options.foldComment && options.foldCommentExplicit && (style == SCE_PAWN_COMMENTLINE)) {
			if ((ch == '/') && (chNext == '/')) {
		...
		if (options.foldPreprocessor && (style == SCE_PAWN_PREPROCESSOR)) {
			if (ch == '#') {
				unsigned int j = i + 1;
...
void LexerPawn::EvaluateTokens(std::vector<std::string> &tokens) {
...
bool LexerPawn::EvaluateExpression(const std::string &expr, const std::map<std::string, std::string> &preprocessorDefinitions) {
...
LexerModule lmPawn(SCLEX_PAWN, LexerPawn::LexerFactoryPawn, "pawn", pawnWordLists);
